...
class ModelBuilder:
    ...
    @staticmethod
    async def _get_model_and_provider_info(model_id: str) -> Optional[Dict[str, Any]]:
        """
        从数据库获取模型和供应商信息
        ...
        try:
            ...
            if missing_fields:
                raise ModelConfigError(
                    model_id, 
                    f"模型配置缺少必要字段: {', '.join(missing_fields)}"
                )
            ...
            # 处理数据库中的默认参数
            if model_info.get("default_params") and isinstance(model_info["default_params"], dict):
                default_params = model_info["default_params"]
                logger.debug(f"模型 {model_id} 的默认参数: {default_params}")
                # 只合并ChatOpenAI支持的参数
                supported_params = {
                    "temperature", "max_tokens", "top_p", "frequency_penalty", 
                    "presence_penalty", "n", "stop", "stream", "logit_bias",
                    "max_retries", "request_timeout", "seed", "response_format"
                }
                filtered_params = {k: v for k, v in default_params.items() if k in supported_params}
            ...
            logger.info(f"为模型 {model_id} 构建ChatOpenAI实例，参数: {list(chat_params.keys())}")
