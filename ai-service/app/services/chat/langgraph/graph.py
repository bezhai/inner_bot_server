...
def create_chat_graph():
    ...
    graph.add_conditional_edges(
        "model_call",
        should_continue_with_tools,
        {"tool_execution": "tool_execution", "output_processing": "output_processing"},
    )

    graph.add_conditional_edges(
        "tool_execution",
        should_continue_after_tools,
        {"model_call": "model_call", "output_processing": "output_processing"},
    )
    ...
    async def stream_execute(
        ...
        try:
            ...
            async for chunk in self.graph.astream(
                initial_state,
                stream_mode=["updates", "custom"],  # 组合模式
            ):
...
# 全局图执行器实例
_chat_graph_executor: ChatGraphExecutor | None = None
