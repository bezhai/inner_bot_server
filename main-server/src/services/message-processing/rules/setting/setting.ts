...
import {
    getAvailableModelsForChat,
    getAvailablePromptsForChat,
    getChatAIConfig,
                ...
                new Column('column_4')
                    .addElements(
                        new SelectComponent('select_prompt')
                            ...
                            .setOptions(
                                validPrompts.map(
                                    (prompt) => new SelectOption(prompt.name, prompt.prompt_id),
                                ),
                            ),
                    )
                    .setVerticalAlign('center'),
                ...
                new Column('column_6')
                    .addElements(
                        new SelectComponent('enable_search')
                            ...
                            .setOptions([
                                new SelectOption('开启', 'true'),
                                new SelectOption('关闭', 'false'),
                            ]),
                    )
                    .setVerticalAlign('center'),
            ),
            new TableButtonComponent('table_button', 'table_button', 'form_submit')
                .setText('提交')
                .addValue({
                    type: SetLLMConfig,
                }),
