import { CompletionRequest, SearchWithAIResponse, WebSearchResult } from '../../../types/ai';
...
export async function fetchChatCompletion(payload: CompletionRequest): Promise<Response> {
    try {
        ...
        if (axios.isAxiosError(error)) {
            throw new Error(
                `Chat completion request failed with status code: ${error.response?.status}`,
            );
        }
        throw error;
    }
...
export async function searchWebWithAI(message: string): Promise<WebSearchResult | null> {
    try {
        ...
        if (axios.isAxiosError(error)) {
            throw new Error(
                `Model list request failed with status code: ${error.response?.status}`,
            );
        }
        throw error;
    }
