...
export async function setChatModel(chatId: string, modelId: string): Promise<boolean> {
  try {
    ...
    // 获取模型信息
    const model = await AIModelRepository.findOne({ where: { model_id: modelId, is_active: true } });
    if (!model) {
      return false;
    }
    ...
    // 查找现有映射
    let mapping = await ChatModelMappingRepository.findOne({ where: { chat_id: chatId, is_active: true } });
...
export async function setChatPrompt(chatId: string, promptId: string): Promise<boolean> {
  try {
    ...
    // 获取提示词信息
    const prompt = await AIPromptRepository.findOne({ where: { prompt_id: promptId, is_active: true } });
    if (!prompt) {
      return false;
    }
    ...
    // 查找现有映射
    let mapping = await ChatPromptMappingRepository.findOne({ where: { chat_id: chatId, is_active: true } });
