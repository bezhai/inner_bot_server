...
export class MemoryClient {
    ...
    async quickSearch(
        ...
        try {
            ...
            console.info(`Memory quick_search成功: ${results.length} 条结果`);
            return results;
            ...
            if (axios.isAxiosError(error)) {
                ...
                if (axiosError.code === 'ECONNABORTED') {
                    console.warn(`Memory服务超时: ${this.timeout / 1000}秒`);
                } else if (axiosError.response) {
                    console.error(`Memory服务HTTP错误: ${axiosError.response.status} - ${axiosError.response.data}`);
                } else {
                    console.error(`Memory服务网络错误: ${axiosError.message}`);
                }
            } else {
                console.error(`Memory服务调用失败: ${error instanceof Error ? error.message : String(error)}`);
            }
            return [];
        }
    }
    ...
    async historyMessages(
        ...
        try {
            ...
            console.info(`Memory history_messages成功，获取到 ${data.total_count || 0} 条消息`);
            return data;
