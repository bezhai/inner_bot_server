...
export class ChatPromptService {
    ...
    static async getPrompt(param: PromptGeneratorParam): Promise<string> {
        try {
            ...
            console.info('主提示词获取成功', { promptId: 'main' });
            return renderedPrompt;

        } catch (error) {
            console.error('获取主提示词失败', { error: error instanceof Error ? error.message : String(error) });
            throw error;
        }
    }
    ...
    static async getBangumiPrompt(): Promise<string> {
        try {
            ...
            console.info('番剧提示词获取成功', { promptId: 'bangumi' });
            return renderedPrompt;

        } catch (error) {
            console.error('获取番剧提示词失败', { error: error instanceof Error ? error.message : String(error) });
            throw error;
        }
    }
}
