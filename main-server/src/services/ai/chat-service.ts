    ...
    static async* processChatSse(
        ...
        try {
            ...
            try {
                ...
                console.info(`消息锁定成功: ${request.message_id}`);
            } catch (error) {
                console.warn(`消息加锁失败: ${request.message_id}`, { error });
                // 即使加锁失败也继续处理
            }
            ...
            console.error('SSE聊天处理失败', { error, messageId: request.message_id });
            yield { step: Step.FAILED };
        } finally {
            // 解锁
            try {
                await del(lockKey);
                console.info(`消息解锁成功: ${request.message_id}`);
            } catch (error) {
                console.warn(`消息解锁失败: ${request.message_id}`, { error });
            }
