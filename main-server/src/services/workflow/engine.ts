        ...
        while (currentNodeId) {
            ...
            try {
                switch (currentNode.type) {
                    case NodeType.START:
                        currentNodeId = await this.executeStartNode(
                            currentNode as StartNodeConfig,
                            context,
                        );
                        break;
                    case NodeType.END:
                        currentNodeId = await this.executeEndNode(
                            currentNode as EndNodeConfig,
                            context,
                        );
                        break;
                    case NodeType.REGULAR:
                        currentNodeId = await this.executeRegularNode(
                            currentNode as RegularNodeConfig,
                            context,
                        );
                        break;
                    case NodeType.CONDITION:
                        currentNodeId = await this.executeConditionNode(
                            currentNode as ConditionNodeConfig,
                            context,
                        );
                        break;
                    default:
