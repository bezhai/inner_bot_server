...
export async function searchMessageByRootId(rootId: string, messageType = "text", limit = 6) {
    const collection = await getMessageCollection();
    return collection.find({ root_id: rootId, message_type: messageType, is_delete: false }, { sort: { create_time: -1 }, limit });
}