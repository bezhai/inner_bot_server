...
async def stream_chat(message_id: str) -> AsyncGenerator[ChatStreamChunk, None]:
    ...
    if pre_blocking:
        ...
        complexity = (
            complexity_result.complexity if complexity_result else Complexity.SIMPLE
        )
        logger.info(f"复杂度路由: complexity={complexity.value}")

        async for chunk in _build_and_stream(message_id, complexity, gray_config):
            yield chunk
    else:
        # === 并行模式：pre 在后台运行，主模型同时流式生成 ===
        logger.info(f"并行模式启动: message_id={message_id}")
        raw_stream = _build_and_stream(message_id, Complexity.SIMPLE, gray_config)
