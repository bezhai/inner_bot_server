...
deploy:
	...
	docker compose build
	# 先更新基础设施服务
	docker compose up -d --no-deps redis mongo postgres elasticsearch meme nginx qdrant
	sleep 5
	# 更新日志相关服务
...
deploy-live:
	...
	docker compose build
	# 先更新基础设施服务（仅当有变更时）
	docker compose up -d --no-deps --no-recreate redis mongo postgres elasticsearch meme nginx qdrant
	sleep 5
	# 更新日志相关服务（仅当有变更时）
