...
- 分层与组件
  - 应用层
    - Main Server（Node.js/TypeScript/Koa）：飞书集成、规则处理、媒体处理、HTTP API（README.md:27-33；main-server/package.json:7-13）。
    - AI Service（Python/FastAPI）：对话引擎、工具调用、记忆管理、向量检索（README.md:27-33；ai-service/app/main.py:16-35）。
...
- 关键子系统
  ...
  - 记忆系统与向量检索：Qdrant 初始化在 AI 应用生命周期中进行（ai-service/app/main.py:21；app/services/qdrant 目录）。
...
- Main Server（Node/TS）
  - 启动：`npm run start`（ts-node）（main-server/package.json:7-9）。
  - 构建：`npm run build`（tsc + tsc-alias）（main-server/package.json:9）。
  - 代码检查/修复：`npm run lint`、`npm run lint:fix`（main-server/package.json:11-12）。
  - 格式化：`npm run format`（Prettier）（main-server/package.json:10）。
- AI Service（Python/FastAPI）
  - 依赖与工具：`pyproject.toml` 指定 fastapi、sqlalchemy、qdrant-client、arq、ruff、pytest 等（ai-service/pyproject.toml:6-41, 56-69）。
  - 包索引镜像与策略：`uv.toml`（aliyun 镜像；允许 insecure host mirrors.aliyun.com）（ai-service/uv.toml:1-7）。
  - Worker 启动（容器内示例）：`uv run --no-sync arq app.workers.unified_worker.UnifiedWorkerSettings`（docker-compose.yml:140-146）。

## 3. 代码风格与最佳实践（项目内显式配置）

- Python（AI Service）
  - Ruff（lint+format）：选择 E/W/F/I/B/C4/UP；行宽 88；per-file-ignores；quote-style=double（ai-service/ruff.toml:3-12, 44-68, 70-72）。
  - 类型检查：Pyright（basic），缺失导入警告、虚拟环境配置与路径（ai-service/pyrightconfig.json:14-28）。
- TypeScript（Main Server）
  - ESLint：基于 @eslint/js 推荐与 @typescript-eslint 插件；关闭若干严格规则；警告级 no-unused-vars/no-console；忽略 dist/node_modules（main-server/eslint.config.js:5-59）。
  - Prettier：lint-staged 针对 src/**/*.ts 写入格式（main-server/package.json:14-18）。
- 路径别名（现状与建议）
...
- 框架与约定（AI Service）
  - Pytest 配置：tests 目录；文件/类/函数命名约定；异步模式；覆盖率输出到 term 与 htmlcov；markers: slow/integration/unit/api（ai-service/pytest.ini:1-19）。
  - 运行建议：在 ai-service 环境中执行 `pytest`（覆盖率阈值当前为 0，用于生成报告而不阻塞）（ai-service/pytest.ini:11-14）。
- 依赖分组包含测试工具：pytest、pytest-asyncio、pytest-xdist、pytest-cov 等（ai-service/pyproject.toml:56-69）。
- Main Server：仓库内未发现显式测试脚本/配置（基于当前文件与 package.json）。

## 5. 安全与数据保护

- 认证与鉴权
  - Bearer 鉴权中间件：校验 `Authorization: Bearer <token>` 与 `INNER_HTTP_SECRET`（main-server/src/middleware/auth.ts:6-29）。
  - 内部通信密钥：AI Service 配置项包含 `inner_http_secret`（ai-service/app/config/config.py:24-25）。
- 输入验证
  - 请求体验证/查询参数验证中间件：统一字段规则（必填、类型、长度、正则、自定义）（main-server/src/middleware/validation.ts:35-80, 85-144）。
  - 示例规则：图片处理与 base64 上传字段约束（main-server/src/middleware/validation.ts:147-200）。
- 速率限制
  - 互斥与时间窗口：令牌队列 + async-mutex（main-server/src/utils/rate-limiting/rate-limiter.ts:3-14, 22-57）。
- 追踪与上下文
  - TraceId 传播：UUID 或请求头 `x-trace-id`，通过 AsyncLocalStorage 贯穿中间件链（main-server/src/middleware/trace.ts:5-13）。
  - AI Service HeaderContextMiddleware：用于请求头上下文（ai-service/app/main.py:30-35）。
- 日志
  - AI Service：JSON 结构化日志，控制台与文件（滚动），路径 `/logs/ai-service/app.log`（ai-service/app/config/logging_config.json:5-25, 27-53）。
...
- 统一环境文件
  - `.env`：主服务、AI 服务、基础设施均从根目录 `.env` 加载（docker-compose.yml:37-38, 100；ai-service/app/config/config.py:47-49；scripts/* 加载 .env）。
- AI Service 应用配置（pydantic Settings）
  - Redis/PG/Qdrant/搜索与对接凭据、主服务 URL、Langfuse、L2/L3 策略、长期任务批量与锁（ai-service/app/config/config.py:5-46）。
- 生命周期与初始化
  - AI Service 应用启动时初始化 Qdrant 集合（ai-service/app/main.py:21）。
- 数据库 schema
